...
const getEmployeeDetails = async () => {
  // const employeeUrl = `/v1/employee/getemployee/${userInfo.userTypeId}`;
  const employeeUrl = `/v1/employee/profile/${userInfo.userTypeId}`;
  try {
    ...
    if (data.status === 'success') {
      const [employeeName] = document.getElementsByClassName(
        'text-primary m-0',
      );
